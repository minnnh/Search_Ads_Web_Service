...
*/
    private double calculateTFIDF(Long adId, String term, int docLength) {
        ...
        // Create a custom transcoder with decompression disabled
        SerializingTranscoder transcoder = new SerializingTranscoder() {
            @Override
            protected byte[] decompress(byte[] in) {
                return in;  // Disable decompression
            }
        };

        try {
            // Connect to the Memcached servers
            MemcachedClient tfClient = new MemcachedClient(new InetSocketAddress("127.0.0.1", 11220));
            MemcachedClient dfClient = new MemcachedClient(new InetSocketAddress("127.0.0.1", 11221));

            // Store and retrieve data using the custom transcoder
            String tf = (String) tfClient.get(tfKey, transcoder);
            System.out.println("tf = " + tf);

            String df = (String) dfClient.get(term, transcoder);
            System.out.println("df=" + df);
            ...
            if (tf != null && df != null) {
                ...
                double norm = Math.sqrt(docLength);
                double tfidfScore = (dfScore * tfScore) / norm;
                // System.out.println("tfidfScore = " + tfidfScore);
                return tfidfScore;
            }
	...
	public List<Ad> selectAds(List<String> queryTerms,String device_id, String device_ip, String query_category) {
		...
		try {
			...
			for (String queryTerm : queryTerms) {
				...
				// System.out.println(cache.get(queryTerm).getClass());
//				Set<Long> adIdList = new HashSet<Long>();
//				if (cache.get(queryTerm) instanceof Collection) {
//					adIdList = (Set<Long>)cache.get(queryTerm);
//				}
				 @SuppressWarnings("unchecked")
				 Set<Long> adIdList = (Set<Long>)cache.get(queryTerm);
				// Set<Long> adIdList = new HashSet<Long>(cache.get(queryTerm));
				// Set<Long> adIdList = new HashSet<Long>((Collection<? extends Long>) cache.get(queryTerm));
				// System.out.println("test");
				// Set<Long> adIdList = new HashSet<Long>();
				// adIdList.addAll((Collection<Long>) result);
			...
			for (Long adId : matchedAds.keySet()) {
				...
				if (ad == null || ad.keyWords == null) continue;
				//old relevance score
				...
				if(enableTFIDF) {
					...
					System.out.println("selectAds relevanceScoreTFIDF = " + relevanceScoreTFIDF);
					ad.relevanceScore = relevanceScoreTFIDF == 0 ? relevanceScore : relevanceScoreTFIDF;
					// System.out.println("selectAds relevanceScoreTFIDF = " + ad.relevanceScore);
				} else {
...
//        };
		...
		System.out.println("ad_id_click_key = " + ad_id_click_key);
		@SuppressWarnings("unchecked")
		String ad_id_click_val_str = (String)featureCacheClient.get(ad_id_click_key);
		// String ad_id_click_val_str = (String)featureCacheClient.get(ad_id_click_key, transcoder);
		Double ad_id_click_val = 0.0;
		if (ad_id_click_val_str != null && ad_id_click_val_str!= "") {
			ad_id_click_val_str = ad_id_click_val_str.replaceAll("[^\\d.]", ""); 
		...
		if (ad_id_impression_val_str != null && ad_id_impression_val_str!= "") {
			ad_id_impression_val_str = ad_id_impression_val_str.replaceAll("[^\\d.]", ""); 
		...
		if (query_campaign_id_click_val_str != null && query_campaign_id_click_val_str!= "") {
			query_campaign_id_click_val_str = query_campaign_id_click_val_str.replaceAll("[^\\d.]", ""); 
		...
		if (query_campaign_id_impression_val_str != null && query_campaign_id_impression_val_str!= "") {
			query_campaign_id_impression_val_str = query_campaign_id_impression_val_str.replaceAll("[^\\d.]", ""); 
		...
		if (query_ad_id_click_val_str != null && query_ad_id_click_val_str!= "") {
			query_ad_id_click_val_str = query_ad_id_click_val_str.replaceAll("[^\\d.]", ""); 
		...
		if (query_ad_id_impression_val_str != null && query_ad_id_impression_val_str!= "") {
			query_ad_id_impression_val_str = query_ad_id_impression_val_str.replaceAll("[^\\d.]", ""); 
		...
		if(query_category != null && query_category == ad.category) {
